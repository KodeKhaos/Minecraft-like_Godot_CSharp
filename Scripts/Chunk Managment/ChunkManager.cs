...
public partial class ChunkManager : Node
{
	...
	public Dictionary<Chunk, Vector2I> _chunkToPosition = new();
	public Dictionary<Vector2I, Chunk> _positionToChunk = new();
	...
	private readonly Dictionary<Vector2I, Block[,,]> _chunkBlocks = new();
	private readonly object _chunkBlocksLock = new();
	...
	public Block[,,] GetChunkBlocks(Vector2I chunkPos)
	{
		lock (_chunkBlocksLock)
		{
			return _chunkBlocks.TryGetValue(chunkPos, out var blocks) ? blocks : null;
		}
	}

	public void SetChunkBlocks(Vector2I chunkPos, Block[,,] blocks)
	{
		lock (_chunkBlocksLock)
		{
			_chunkBlocks[chunkPos] = blocks;
		}
	}

	public void GenerateChunkAsync(Vector2I chunkPos)
	{
		ThreadPool.QueueUserWorkItem(_ =>
		{
			var blocks = TerrainManager.Instance.GetChunkData(chunkPos);
			SetChunkBlocks(chunkPos, blocks);
			// Notify the chunk to update its mesh on the main thread
		});
	}
}
